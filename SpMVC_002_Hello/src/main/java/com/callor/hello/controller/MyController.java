...
public class MyController {
	 ...
	 * 		DELETE : 삭제(DELETE)
	 * 
	 * RequestMapping이 같고, RequestMethod가 같은 경우는 절대 혀용하지 않는다.
	 * 
	 ...
	 * 		다만, 이미지와 같이 큰 데이터를 전송하게 되면 네트워크에 문제가 발생할 수 있어서 설정에서 일부 크기를 제한하기도 한다
	 * 
	 * 서버입장에서 'GET: http://localhost:8080/hello/insert?name=홍길동'으로 요청을 받았으면
	 * 	@RequestMapping(value="/insert", method="GET")를 처리하는 method가 있어야 한다.
	 * 
	 * Mapping(value="/insert")가 있는데 method = "POST"만 있을 경우
	 */
	 ...
	 * 
	 * 그런데, Java method의 변수를 String형이 아닌 것으로 숫자형, Boolean형 등으로 설정을 하면 
	 * 	Reflection이 실행되는 동안에 String형 데이터를 설정된 데이터 type으로 형변환 과정이 실행된다
	 *		이 과정에서 형변환의 Exception이 발생하면 Reflection 코드가 중단되고 Spring의 Dispatcher는 Web의 400 status code를 응답해 버린다.
	 *
	 *		또한 구체적인 어떤 문제가 발생했는 지 알려주지 않는다.
	 */
